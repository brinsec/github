...
class ClassificationService {
    classifyRepository(repository, categories) {
    ...
    calculateConfidence(repository, criteria) {
        let score = 0;
        let maxScore = 0;
        // 语言匹配 (权重: 40%)
        ...
        if (criteria.topics && criteria.topics.length > 0) {
            ...
            if (repository.topics && repository.topics.length > 0) {
                const matchingTopics = repository.topics.filter(topic => criteria.topics.some(criteriaTopic => topic.toLowerCase().includes(criteriaTopic.toLowerCase()) ||
    ...
    getClassificationReasons(repository, criteria) {
        const reasons = [];
        // 语言匹配原因
        ...
        if (criteria.keywords && criteria.keywords.length > 0) {
            ...
            const name = repository.name.toLowerCase();
            const fullName = repository.full_name.toLowerCase();
            const matchedKeywords = criteria.keywords.filter(keyword => description.includes(keyword.toLowerCase()) ||
                name.includes(keyword.toLowerCase()) ||
                fullName.includes(keyword.toLowerCase()));
